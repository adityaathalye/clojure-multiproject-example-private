(ns usermanager.router.core-grug-test
  (:require
   [clojure.test :refer [are deftest testing use-fixtures]]
   [usermanager.router.core :as urc]
   [usermanager.test-utilities :as tu]))

#_(use-fixtures :each (partial tu/with-test-db ::db))

(def system-atom (atom nil))

(use-fixtures :each (partial tu/setup-teardown-grug! system-atom))

(def db #(get-in @system-atom
